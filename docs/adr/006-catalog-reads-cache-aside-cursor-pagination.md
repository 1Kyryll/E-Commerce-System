...
**Decision.** Request for a catalog hits the Redis cache first(name, description, image, stock is left for DB query), if nothing is found, then hit the DB directly. Use cursor pagination for queryinh chunks of data.
