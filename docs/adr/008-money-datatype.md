...
**Consequences.** One rule is non-negotiable regardless of the storage choice: **never store an amount without its currency in the same row**. A `NUMERIC price_amount` column without a paired `currency` column is a future bug waiting for the day a second currency appears.
