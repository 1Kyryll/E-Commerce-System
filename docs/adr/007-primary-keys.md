...
**Decision.** Every row gets a UUIDv7 primary key, stored using Postgres's native `uuid` type (16 bytes, binary). No `bigserial`, no string-form storage, no opaque tokens layered on top.
